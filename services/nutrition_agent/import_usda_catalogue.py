...
async def import_catalogue(page_size: int, max_pages: int | None) -> tuple[int, int]:
    ...
    try:
        ...
        while max_pages is None or page_number <= max_pages:
            foods = await provider.list_foods(page_number, page_size, APPROVED_DATA_TYPES)
            if not foods:
                break
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--all", action="store_true", help="Import every page of approved USDA datasets."
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    print(f"Completed USDA catalogue import: {count} foods imported, {skipped_count} incomplete foods skipped.")