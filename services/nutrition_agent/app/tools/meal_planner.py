...
MEAL_TEMPLATES = {
    "omnivore": {
        "breakfast": [
            ...
            {
                "name": "Greek yogurt protein bowl with berries",
                "calories": 430,
                "protein_g": 40,
                "carbs_g": 42,
                "fat_g": 10,
    ...
    "vegetarian": {
        "breakfast": [
            {
                "name": "Vegetable omelet with cheese",
                "calories": 380,
                "protein_g": 22,
                "carbs_g": 15,
                "fat_g": 24,
            ...
            {
                "name": "Cottage cheese protein bowl with berries",
                "calories": 390,
                "protein_g": 36,
                "carbs_g": 38,
                "fat_g": 9,
    ...
    "vegan": {
        "breakfast": [
            ...
            {
                "name": "Oatmeal with almond milk and banana",
                "calories": 320,
                "protein_g": 10,
                "carbs_g": 58,
                "fat_g": 8,
            ...
            {
                "name": "Tofu and soy yogurt protein bowl",
                "calories": 440,
                "protein_g": 32,
                "carbs_g": 40,
                "fat_g": 16,
        ...
        "lunch": [
            {
                "name": "Buddha bowl with quinoa and chickpeas",
                "calories": 450,
                "protein_g": 18,
                "carbs_g": 65,
                "fat_g": 14,
    ...
    "pescatarian": {
        "breakfast": [
            {
                "name": "Smoked salmon with cream cheese on bagel",
                "calories": 420,
                "protein_g": 22,
                "carbs_g": 35,
                "fat_g": 20,
        ...
        "dinner": [
            {
                "name": "Baked cod with quinoa and vegetables",
                "calories": 450,
                "protein_g": 40,
                "carbs_g": 40,
                "fat_g": 12,
...
def _matches_exclusion(ingredients: set[str], exclusions: set[str]) -> bool:
    ...
    return any(
        exclusion == ingredient or exclusion in ingredient or ingredient in exclusion
...
def eligible_meal_catalog(
    dietary_preference: str = "omnivore",
    *,
    allergies: list[str] | None = None,
    dietary_restrictions: list[str] | None = None,
) -> list[dict]:
    """Return only catalog meals proven safe for a profile, with stable server IDs."""
    exclusions = {
        item.strip().casefold()
    ...
    preference = (
        dietary_preference
        if dietary_preference in MEAL_TEMPLATES
        else "omnivore"
    )
    catalog = []
    for meal_type, options in MEAL_TEMPLATES[preference].items():
        for index, option in enumerate(_safe_options(options, exclusions)):
            catalog.append({"id": f"{preference}:{meal_type}:{index}", "meal_type": meal_type, **option})
    return catalog


def catalog_meal(catalog_id: str, catalog: list[dict]) -> dict | None:
    """Resolve an ID only within the eligible catalog built for this request."""
    return next((item for item in catalog if item["id"] == catalog_id), None)
...
def generate_meal_plan(
    ...
    if meals_per_day >= 1:
        ...
        if breakfast_options:
            ...
            breakfast = min(
                breakfast_options,
                key=lambda x: abs(x["calories"] - target_breakfast_cal),
            )
    ...
    if meals_per_day >= 2:
        ...
        if lunch_options:
            ...
            lunch = min(
                lunch_options, key=lambda x: abs(x["calories"] - target_lunch_cal)
            )
    ...
    if meals_per_day >= 3:
        ...
        if dinner_options:
            ...
            dinner = min(
                dinner_options, key=lambda x: abs(x["calories"] - target_dinner_cal)
            )
    ...
    if meals_per_day >= 4:
        ...
        if snack_options:
            ...
            snack = min(
                snack_options, key=lambda x: abs(x["calories"] - target_snack_cal)
            )
