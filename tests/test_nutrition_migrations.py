...
@pytest.mark.asyncio
async def test_second_imported_food_cache_seed_uses_its_own_sentinel():
    connection = AsyncMock()
    result = Mock()
    result.scalar_one.return_value = True
    connection.exec_driver_sql.return_value = result
    ...
    assert (
        await imported_food_cache_seed_exists(
            connection, "004_seed_nutrition_food_cache.sql"
        )
        is True
    )
    assert (
        "provider_food_id = '2709972'" in connection.exec_driver_sql.await_args.args[0]
    )
...
async def test_run_migrations_skips_imported_seed_before_reading_its_contents(
    ...
    seed = migration_dir / "004_seed_nutrition_food_cache.sql"
