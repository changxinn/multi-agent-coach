...
from app.api.routes.recovery import router, get_current_user, get_db
...
PAYLOADS = [
    ('sleep-logs', {'user_id': 1, 'duration_minutes': 480, 'quality': 4, 'notes': None}),
    ('check-ins', {'user_id': 1, 'energy': 7, 'soreness': 3, 'stress': 4, 'notes': None}),
    ('assessments', {'user_id': 1, 'status': 'green', 'score': 2, 'response': {'message': 'Rest'}, 'tool_trace': []}),
]
...
def test_create_update_list_delete(api_client, kind, payload):
    ...
    for method, url, code in [('post', f'/api/recovery/{kind}', 201), ('put', f'/api/recovery/{kind}/12', 200)]:
...
def test_access_and_validation(api_client, kind, payload):
    ...
    assert client.post(f'/api/recovery/{kind}', json={**payload, 'user_id': 0}).status_code == 422
    ...
    for method, suffix in [('get', ''), ('post', ''), ('put', '/1'), ('delete', '/1')]:
        assert client.request(method, f'/api/recovery/{kind}{suffix}', json=payload).status_code == 403
...
@pytest.mark.parametrize('kind,payload', [
    ...
    ('assessments', {'user_id': 1, 'status': 'invalid', 'score': -1, 'response': [], 'tool_trace': {}}),
])
